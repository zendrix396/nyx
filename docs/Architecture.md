...
```mermaid
flowchart TB
    U["USER<br/>(Voice, Text, Hotkey, Implicit Feedback)"] --> O["Orchestrator (Rust Core)<br/>- Manages task lifecycle<br/>- Manages state (IDLE, etc.)"]
    U --> F["Feedback Loop (RL Tuner)<br/>- Collects (Task, Actions, Outcome)<br/>- (Future) Fine-tunes the model"]
    ...
    O --> T["Tooling Module<br/>(MCP Client/Host)<br/>- Low-level I/O (Mouse/Keys)"]
